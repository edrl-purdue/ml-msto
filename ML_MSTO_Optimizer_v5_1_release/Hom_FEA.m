% HOMOGENIZED FINITE ELEMENT ANALYSIS FOR ANY SCALE
function [U,Ue_int,Fe_int] = Hom_FEA(KE_Hom,scale,F,U,N)
    ...
    [K_Hom, K_Hom_nele, ~] = Prepare_K([],scale,[],[],[],KE_Hom,[],edof,[]); % Case 3: Calling Prepare_K([],scale,[],[],[],KE_Hom,[],edof,[]) is Homogenized stiffness matrix only
    
    %% SOLVE FEA
    if any(scale.phys == 2) % 2 = Thermal
        F{2}(N{2}==1) = F{2}(N{2}==1) - K_Hom{2,2}(N{2}==1,N{2}==0)*U{2}(N{2}==0);
        for LC = 1:length(U{2}(1,:))
            try
                R = chol(K_Hom{2,2}(N{2}(:,LC)==1,N{2}(:,LC)==1));
                U{2}(N{2}(:,LC)==1,LC) = R\(R'\F{2}(N{2}(:,LC)==1,LC)); % solve Cholesky thermal problem
            catch
                U{2}(N{2}(:,LC)==1,LC) = K_Hom{2,2}(N{2}(:,LC)==1,N{2}(:,LC)==1)\F{2}(N{2}(:,LC)==1,LC); % solve standard thermal problem
            end
        end
    end
    ...
    if all(scale.phys == [1 2]) % 1,2 = Thermomechanical
        Ft = zeros(size(F{1}));
        Ft(N{1}==1) = K_Hom{2,1}(N{1}==1,N{2}==1)*U{2}(N{2}==1); % solve thermomechanical problem
        F{1} = F{1} + Ft; % add thermally-induced forces to mechanical load
    end
    if any(scale.phys == 1) % 1 = Mechanical
        F{1}(N{1}==1) = F{1}(N{1}==1) - K_Hom{1,1}(N{1}==1,N{1}==0)*U{1}(N{1}==0);
        for LC = 1:length(U{1}(1,:))
            try
                R = chol(K_Hom{1,1}(N{1}(:,LC)==1,N{1}(:,LC)==1));
                U{1}(N{1}(:,LC)==1,LC) = R\(R'\F{1}(N{1}(:,LC)==1,LC)); % solve Cholesky mechanical problem
            catch
                U{1}(N{1}(:,LC)==1,LC) = K_Hom{1,1}(N{1}(:,LC)==1,N{1}(:,LC)==1)\F{1}(N{1}(:,LC)==1,LC); % solve standard mechanical problem
            end
        end
    end
    ...
    if any(scale.phys == 1) % 1 = Mechanical
        ...
        if length(F{1}(1,:)) == 1
            for i = 1:scale.nele
                ...
                Fe_int{1}(i,:) = reshape(K_Hom_nele{1,1}(i,:),[scale.nen*scale.dim,scale.nen*scale.dim])*Ue_int{1}(i,:)'; % solve for internal forces
            end
        end
    end
    if any(scale.phys == 2) % 2 = Thermal
        ...
        if length(F{2}(1,:)) == 1
            for i = 1:scale.nele
                ...
                Fe_int{2}(i,:) = reshape(K_Hom_nele{2,2}(i,:),[scale.nen,scale.nen])*Ue_int{2}(i,:)'; % solve for internal fluxes
            end
        end
    end
